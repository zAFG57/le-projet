...
class CreateAccount extends Database {
    protected function isEmailAlreadyUsing($email) {
        return (parent::sqlSelect('SELECT id FROM users WHERE email=?', 's', $email)->num_rows === 0);
    }

    protected function addUser($id, $username, $email, $password) {
        $passHash = password_hash($password, PASSWORD_DEFAULT);
        return parent::sqlInsert($db, 'INSERT INTO users VALUES (?, ?, ?, ?, 0, 0)', 'isss', $id, $username, $email, $passHash);
    }

    protected function createId($email) {
